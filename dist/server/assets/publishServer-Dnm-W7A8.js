...
const savePublishConfig = createServerFn({
  ...
  try {
    ...
    const gitPresent = spawnSync("git", ["rev-parse", "--git-dir"], {
      cwd: process.cwd(),
      encoding: "utf8"
    });
    ...
    if (inProductionMode) {
      const {
        commitFiles
      } = await import("./github-CKcpqj8A.js");
      const {
        resolveTokenSource
      } = await import("./tokenStore-Dpx2yaa_.js");
      if (resolveTokenSource()?.token) {
        const SYNC_FILES = ["data/content.json", "data/players.json", "data/gamemodes.json", "data/economy.json", "data/event.json", "data/homepage.json", "data/tier-tagger.json"];
        const files = [];
        ...
        if (files.length > 0) {
          await commitFiles(files, `[admin] Publish config saved (${(/* @__PURE__ */ new Date()).toISOString().slice(0, 10)})`);
        }
      }
    }
  } catch {
