...
import { getRepoStatus, commitFiles } from "./github-CKcpqj8A.js";
import { resolveTokenSource, persistToken, eraseToken } from "./tokenStore-Dpx2yaa_.js";
import { readRepoConfig } from "./repoConfigUtil-DmRyhrlE.js";
import { m as markAlreadyCommitted, g as getBackupStatus, s as setBackupEnabled, a as setDebounceMs, f as flushBackupNow } from "./miningBackup-C2ru7u1S.js";
...
const checkGitHubConnection = createServerFn({
  ...
  try {
    const repoRes = await fetch(`${BASE}/repos/${cfgOwner}/${cfgRepo}`, {
      headers: buildGHHeaders(token)
    });
    if (repoRes.ok) {
      out.repoExists = true;
  ...
  if (out.repoExists) {
    try {
      const brRes = await fetch(`${BASE}/repos/${cfgOwner}/${cfgRepo}/git/refs/heads/${cfgBranch}`, {
        headers: buildGHHeaders(token)
      });
...
const getGitDiagnostics = createServerFn({
  ...
  if (!repoCheck.ok) {
    const jsonChecks2 = Object.values(FILE_MAP).map(({
      file
    }) => {
      const raw = readRaw(file);
      if (!raw) return {
    ...
    try {
      ...
      return {
        ...
        totalPending: 0,
        statusLines: [],
        jsonChecks: jsonChecks2,
        apiMode: true,
        ...status.connected ? {} : {
          gitError: "Could not connect to GitHub API — check your token."
        }
      };
    } catch (e) {
      return {
        ...
        jsonChecks: jsonChecks2,
        apiMode: true,
        gitError: `GitHub API error: ${e.message}`
      };
    }
  }
...
const SYNC_DATA_FILES = ["data/ads-config.json", "data/content.json", "data/economy.json", "data/event.json", "data/gamemodes.json", "data/growth.json", "data/homepage.json", "data/mining-access.json", "data/mining-community.json", "data/mining-users.json", "data/players.json", "data/shop-items.json", "data/shop-purchases.json", "data/sync-history.json", "data/tier-tagger.json", "data/tournaments.json", "credentials.yml", "admin.yml"];
async function pushAllDataViaApi(logs) {
  const {
    existsSync,
    readFileSync: fsRead
  ...
  try {
    const result = await commitFiles(files, `[admin] Sync data from production (${(/* @__PURE__ */ new Date()).toISOString().slice(0, 10)})`);
    ...
    logs.push(`✗ GitHub API push failed: ${e.message}`);
    return {
      success: false,
      action: "error",
      logs,
...
const fixGitDivergence = createServerFn({
  ...
  const repoPresent = spawnSync("git", ["rev-parse", "--git-dir"], {
    cwd,
    encoding: "utf8"
  });
...
const backupMiningData = createServerFn({
  ...
  if ((repoPresent.status ?? 1) !== 0) {
    ...
    const files = MINING_FILES_REL.filter((f) => existsSync(resolve(cwd, f))).map((f) => ({
      path: f,
      content: fsRead(resolve(cwd, f), "utf8")
    }));
    if (files.length === 0) {
      return {
        success: false,
        alreadyBackedUp: false,
        logs: [...logs, "✗ No mining files found."],
        filesCommitted: []
      };
    }
    logs.push(`→ Pushing ${files.length} mining file(s) via API…`);
    try {
      const msg = `Mining data snapshot (${(/* @__PURE__ */ new Date()).toISOString().slice(0, 16).replace("T", " ")} UTC)`;
      ...
      logs.push(`✗ API push failed: ${e.message}`);
      return {
        success: false,
        alreadyBackedUp: false,
        logs,
